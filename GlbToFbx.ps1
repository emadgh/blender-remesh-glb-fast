...
$xaml = @'
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" Title="GLB to FBX • Unity" Height="650" Width="820" MinHeight="570" MinWidth="700" WindowStartupLocation="CenterScreen" Background="#F5F5F4" FontFamily="Segoe UI">
  <Grid Margin="22">
    ...
    <StackPanel Grid.Row="0" Margin="0,0,0,14">
      <TextBlock Text="GLB → FBX  |  Unity Converter" FontSize="25" FontWeight="SemiBold" Foreground="#171717"/>
      <TextBlock Text="تبدیل مستقیم بدون Remesh؛ متریال و UV حفظ می‌شوند و تکسچرها کنار FBX آنپک می‌شوند." FontSize="13" Foreground="#555" Margin="0,5,0,0" FlowDirection="RightToLeft" HorizontalAlignment="Left"/>
    </StackPanel>
    ...
    <StackPanel Grid.Row="4" Margin="0,0,0,12">
      <WrapPanel>
        <StackPanel Width="190" Margin="0,0,12,8"><TextBlock Text="Texture format"/><ComboBox Name="TextureFormat" SelectedIndex="0"><ComboBoxItem Content="PNG (recommended)" Tag="png"/><ComboBoxItem Content="Original extension" Tag="original"/></ComboBox></StackPanel>
        <CheckBox Name="ExportAnimation" Content="Export animations" IsChecked="True" VerticalAlignment="Center" Margin="0,17,20,8"/>
      </WrapPanel>
      <StackPanel Orientation="Horizontal" Margin="0,8,0,0">
        <Button Name="Start" Content="▶  Convert GLB → FBX" Padding="20,10" Background="#222" Foreground="White" BorderThickness="0" FontWeight="SemiBold"/>
...
$names = 'DropArea','FilesList','AddFiles','RemoveFiles','ClearFiles','OutputPath','BrowseOutput','BlenderPath','BrowseBlender','TextureFormat','ExportAnimation','Start','Cancel','Status','Log'
...
$Start.Add_Click({
    ...
    $script:runFolder = Join-Path $OutputPath.Text ("glb_to_fbx_logs_" + (Get-Date -Format 'yyyyMMdd_HHmmss'))
    [IO.Directory]::CreateDirectory($script:runFolder) | Out-Null
    $format = $TextureFormat.SelectedItem.Tag
    $script:arguments = @('--texture-format', $format)
    if (-not $ExportAnimation.IsChecked) { $script:arguments += '--no-animation' }
